...
Esto significa que el 95% de los usuarios experimentan tiempos de respuesta menores a 198.54ms, cumpliendo nuestro SLO de < 500ms con un margen confortable del 60%.

**b) Utilización de Recursos (Resource Utilization)**

Calculamos el throughput del sistema:

$$Throughput = \frac{356,518 \text{ peticiones}}{51 \text{ segundos}} \approx 6,990 \text{ req/s}$$

Este throughput de ~7,000 req/s demuestra que el sistema puede manejar una carga significativa, más del doble de lo estimado inicialmente.

**c) Capacidad (Capacity)**

Determinamos que el sistema puede soportar al menos 10,000 usuarios concurrentes manteniendo:
- Latencia promedio: 47.1ms
- Latencia p95: 198.54ms (< 500ms threshold)
- Tasa de errores: 0% (< 1% threshold)

#### **2. Fiabilidad (Reliability)**

La fiabilidad se mide como la proporción de peticiones exitosas:

$$Fiabilidad = \frac{Peticiones_{exitosas}}{Peticiones_{totales}} \times 100\% = \frac{356,518}{356,518} \times 100\% = 100\%$$

Con 356,518 peticiones exitosas y 0 fallos, esto indica una fiabilidad perfecta del sistema bajo carga extrema.
...
Con 0 fallos durante las 356,518 peticiones, MTTR = 0, lo que resulta en disponibilidad del 100%.
...
En nuestro caso, verificamos la relación con nuestros datos reales:

$$\lambda = \frac{L}{W} = \frac{10,000}{0.0471} \approx 212,300 \text{ req/s teóricos}$$

El throughput real obtenido fue de 6,990 req/s. Esta diferencia se debe al `sleep(1)` que introducimos intencionalmente en cada iteración, que limita artificialmente la tasa de peticiones para simular el comportamiento de usuarios reales que esperan entre acciones.

Si consideramos el `sleep(1)`:

$$Tiempo_{total\_iteración} = W + sleep = 0.0471s + 1s = 1.0471s$$

$$\lambda_{real} = \frac{10,000}{1.0471} \approx 9,550 \text{ req/s teóricos}$$

Nuestro throughput real de 6,990 req/s es aproximadamente el 73% del teórico, lo cual es razonable considerando la rampa de subida/bajada y las variaciones de red.
...
En nuestras pruebas obtuvimos:
- **Promedio**: 47.1ms
- **Mediana p(50)**: 11.01ms
- **p(90)**: 150.63ms
- **p(95)**: 198.54ms
- **Máximo**: 760.59ms

Análisis de la distribución:
- La mediana (11.01ms) es muy inferior al promedio (47.1ms), indicando que la mayoría de peticiones son muy rápidas
- Solo el 5% de las peticiones superan los 198.54ms
- Incluso el peor caso (760.59ms) está dentro del rango aceptable
- La distribución muestra que el sistema maneja bien la carga, con la mayoría de usuarios experimentando latencias excelentes
...
- **SLI** (Service Level Indicator): La métrica real medida
  - Ejemplo obtenido: `http_req_duration p(95) = 198.54ms`
  
- **SLO** (Service Level Objective): El objetivo interno
  - Ejemplo configurado: `p(95) < 500ms`
  - **Estado**: ✅ Cumplido con 60% de margen
  
- **SLA** (Service Level Agreement): El contrato con clientes
  - Ejemplo: "99.9% de disponibilidad mensual"
  - **Estado**: ✅ Superado (100% de disponibilidad en el test)

El cumplimiento de los SLOs con margen amplio (198.54ms vs 500ms threshold) indica que el sistema está sobredimensionado para la carga actual, lo cual es positivo para futuros crecimientos de tráfico.
---
